...
class PlayControllingCardAssist: UIView {
    ...
    static let AssistHeight = 50.0 + SafeAreaInsetBottom
    
    required init?(coder: NSCoder) { fatalError("init(coder:) has not been implemented") }
    override init(frame: CGRect) {
        let f = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(PlayControllingCardAssist.AssistHeight))
        super.init(frame: f)
        self.backgroundColor = R.color.mu_color_clear()
        let swip = UISwipeGestureRecognizer(target: self, action: #selector(pullerTriggered(_:)))
        swip.direction = .up
...
fileprivate extension PlayControllingCardAssist {
    ...
    func animate(showing show: Bool) {
        ...
        UIView.animate(withDuration: 0.2, delay: 0, options: .curveEaseInOut) {
            ...
            self.h_y = show ? Double(ScreenHeight - PlayControllingCardAssist.AssistHeight) : Double(ScreenHeight)
    ...
    func shakingAnimation()->CAAnimationGroup? {
        ...
        pathAnimation.fromValue = self.puller.h_y + self.puller.h_h / 2
        pathAnimation.toValue = self.puller.h_h / 2
